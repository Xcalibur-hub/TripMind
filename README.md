...
## Project Description
TripMind is an AI-powered travel assistant that generates end-to-end travel itineraries based on user constraints like destination, duration, budget, and interests. 

Unlike standard LLM wrappers, TripMind utilizes a **Multi-Agent Architecture** to guarantee reliability. [cite_start]It integrates an external search API to cross-reference and verify the ground truth of its own suggestions in real-time, fulfilling the hackathon's objective for integrating real-time data for bonus points.
